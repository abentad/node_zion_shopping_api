...
router.get('/products/category', requireAuth, async (req,res)=> {
    ...
    try {
        mysqlConnection.query("SELECT * FROM products WHERE isPending = ? AND category = ?", [pendingString, category] ,(error, rows, fields)=>{
            ...
            const startPoint = (page - 1) * size;
            mysqlConnection.query(`SELECT * FROM products WHERE isPending = ? AND category = ? ORDER BY id DESC LIMIT ${startPoint},${size}`, [pendingString, category] ,(error, rows, fields)=>{
                if(error) {
                    res.status(500).json({message: "something went wrong"});
                    console.log(error);
                }
